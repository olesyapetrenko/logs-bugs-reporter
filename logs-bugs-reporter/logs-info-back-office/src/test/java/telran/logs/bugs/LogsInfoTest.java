...
public class LogsInfoTest {
	...
	static void setUpAll() {
		exceptionsAuthentication = new ArrayList<>(Arrays.asList(
				new LogDto(DATE_TIME, LogType.AUTHENTICATION_EXCEPTION, ARTIFACT_AUTHENTICATION, 0,
		...
		exceptionsAuthorization = new ArrayList<>(Arrays.asList(
				new LogDto(DATE_TIME, LogType.AUTHORIZATION_EXCEPTION, ARTIFACT_AUTHORIZATION, 0, AUTHORIZATION_ERROR),
				new LogDto(DATE_TIME, LogType.AUTHORIZATION_EXCEPTION, ARTIFACT_AUTHORIZATION, 0,
						AUTHORIZATION_ERROR)));
		exceptionsBadRequest = new ArrayList<>(
				Arrays.asList(new LogDto(DATE_TIME, LogType.BAD_REQUEST_EXCEPTION, ARTIFACT_BAD_REQUEST, 0, "")));
		noExceptions = new ArrayList<>(
				Arrays.asList(new LogDto(DATE_TIME, LogType.NO_EXCEPTION, ARTIFACT_NO_EXCEPTION, 20, "result"),
	...
	@Test
	void allNoException() {
		testClient.get().uri(LOGS_TYPE + "?" + TYPE + "=NO_EXCEPTION").exchange().expectStatus().isOk()
				.expectBodyList(LogDto.class).isEqualTo(noExceptions);
	}
	...
	@Test
	void logTypeCount() {
		LogTypeCount[] expected = { new LogTypeCount(LogType.NO_EXCEPTION, noExceptions.size()),
	...
	@Test
	void artifactCount() {
		ArtifactCount[] expected = { new ArtifactCount(ARTIFACT_NO_EXCEPTION, noExceptions.size()),
	...
	void exceptionMostEncountered() {
		LogType[] expected = { LogType.AUTHENTICATION_EXCEPTION, LogType.AUTHORIZATION_EXCEPTION };
