...
//	@Query("select programmer.email as email, count(*) as count from Bug group by programmer.email order by count(*) desc")
//                                          	or (equal)
//	@Query("select programmer.email as email, count(*) as count from Bug b join b.programmer programmer "
//			+ " group by programmer.email order by count(*) desc")
	
	@Query("select programmer.email as email, count(b) as count from Bug b right join b.programmer programmer "
			+ " group by programmer.email order by count(b) desc")
	List<EmailBugsCount> emailBugsCounts();
	
	@Query(value = "select p.email from bugs b join programmers p on b.programmer_id = p.id "
	...
	@Query(value = "select seriousness from bugs group by seriousness order by count(*) desc limit :n_types",
			nativeQuery = true)
	List<Seriousness> seriousnessMostBugs(@Param ("n_types") int nTypes);
	
//returns count of bugs of the given seriousness
