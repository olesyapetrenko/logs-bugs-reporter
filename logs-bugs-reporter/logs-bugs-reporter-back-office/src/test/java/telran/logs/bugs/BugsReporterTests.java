...
public class BugsReporterTests {
	...
	private static final @NotEmpty String DESCRIPTION_CLOSED = DESCRIPTION + BugsReporterImpl.CLOSING_DESCRIPTION
			+ TEST_CLOSE_DESCRIPTION;
	...
	BugDto bugUnAssigned = new BugDto(Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN);
	BugAssignDto bugAssigned2 = new BugAssignDto(Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN,
			PROGRAMMER_ID_VALUE_MOSHE);
	BugAssignDto bugAssigned3 = new BugAssignDto(Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN,
			PROGRAMMER_ID_VALUE_MOSHE);
	BugResponseDto expectedUnAssigned = new BugResponseDto(1, Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN, 0, null,
			BugStatus.OPENNED, OpeningMethod.MANUAL);
	BugResponseDto expectedAssigned2 = new BugResponseDto(2, Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN,
			PROGRAMMER_ID_VALUE_MOSHE, null, BugStatus.ASSIGNED, OpeningMethod.MANUAL);
	BugResponseDto expectedAssigned3 = new BugResponseDto(3, Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN,
			PROGRAMMER_ID_VALUE_MOSHE, null, BugStatus.ASSIGNED, OpeningMethod.MANUAL);
	BugResponseDto expectedAssigned1 = new BugResponseDto(1, Seriousness.BLOCKING,
			DESCRIPTION + BugsReporter.ASSIGNMENT_DESCRIPTION_TITLE, DATE_OPEN, PROGRAMMER_ID_VALUE_MOSHE, null,
			BugStatus.ASSIGNED, OpeningMethod.MANUAL);
	List<BugResponseDto> expectedBugs123 = Arrays.asList(expectedAssigned1, expectedAssigned2, expectedAssigned3);
	BugResponseDto expectedClosed2 = new BugResponseDto(BUG_CLOSE_ID_VALUE, Seriousness.BLOCKING, DESCRIPTION_CLOSED,
			DATE_OPEN, PROGRAMMER_ID_VALUE_MOSHE, LocalDate.now(), BugStatus.CLOSED, OpeningMethod.MANUAL);
	...
	List<BugResponseDto> expectedBugsAfterClose = Arrays.asList(expectedAssigned1, expectedClosed2, expectedAssigned3);

	List<SeriousnessBugCount> seriousnessBugsDistribution = Arrays.asList(
			new SeriousnessBugCount(Seriousness.BLOCKING, 3), new SeriousnessBugCount(Seriousness.CRITICAL, 0),
			new SeriousnessBugCount(Seriousness.MINOR, 0), new SeriousnessBugCount(Seriousness.COSMETIC, 0)
...
//============================ Positive Testing=================================
	...
	private void addProgrammerRequest(ProgrammerDto programmer) {
		testClient.post().uri(BUGS_PROGRAMMERS).contentType(MediaType.APPLICATION_JSON).bodyValue(programmer).exchange()
				.expectStatus().isOk().expectBody(ProgrammerDto.class);
	}

	@Test
	@Order(2)
	void openBug() {
		testClient.post().uri(BUGS_OPEN).contentType(MediaType.APPLICATION_JSON).bodyValue(bugUnAssigned).exchange()
				.expectStatus().isOk().expectBody(BugResponseDto.class).isEqualTo(expectedUnAssigned);
	}
	...
	private void openAssignRequest(BugAssignDto bugAssignDto, BugResponseDto bugResponseDto) {
		testClient.post().uri(BUGS_OPEN_ASSIGN).bodyValue(bugAssignDto).exchange().expectStatus().isOk()
				.expectBody(BugResponseDto.class).isEqualTo(bugResponseDto);
	}

	@Test
	@Order(4)
	void assign() {
		testClient.put().uri(BUGS_ASSIGN).bodyValue(new AssignBugData(1, PROGRAMMER_ID_VALUE_MOSHE, "")).exchange()
				.expectStatus().isOk();
	...
	private void getListBugs(List<BugResponseDto> expectedList, String uriStr) {
		testClient.get().uri(uriStr).exchange().expectStatus().isOk().expectBodyList(BugResponseDto.class)
	...
	void closeBug2() {
		...
		testClient.put().uri(BUGS_CLOSE).contentType(MediaType.APPLICATION_JSON).bodyValue(closeData).exchange()
				.expectStatus().isOk();
	}
	...
	void addArtifact() {
		...
		testClient.post().uri(BUGS_ARTIFACTS).contentType(MediaType.APPLICATION_JSON).bodyValue(artifactDto).exchange()
				.expectStatus().isOk().expectBody(ArtifactDto.class).isEqualTo(artifactDto);
	...
	private void getArrayProgrammers(String uriStr, String[] expected) {
		testClient.get().uri(uriStr).exchange().expectStatus().isOk().expectBody(String[].class).isEqualTo(expected);
	}
	...
	void seriousnessMostBugs() {
		testClient.get().uri(BUGS_SERIOUSNESS_MOST).exchange().expectStatus().isOk().expectBodyList(Seriousness.class)
				.isEqualTo(seriousnessBugsMost);
	}
...
//=================================== Negative Testing=============================
	...
	private void postClientError(String uriStr, Object objExistingId) {
		testClient.post().uri(uriStr).contentType(MediaType.APPLICATION_JSON).bodyValue(objExistingId).exchange()
	...
	void invalidOpenAndAssign() {
		BugAssignDto invalidBugAssignDto = new BugAssignDto(Seriousness.BLOCKING, DESCRIPTION, null, 100000);
	...
	private void postNotFound(Object objNotFoundId, String uriStr) {
		testClient.post().uri(uriStr).contentType(MediaType.APPLICATION_JSON).bodyValue(objNotFoundId).exchange()
				.expectStatus().isNotFound();
	}

	private void putNotFound(Object objNotFoundId, String uriStr) {
		testClient.put().uri(uriStr).contentType(MediaType.APPLICATION_JSON).bodyValue(objNotFoundId).exchange()
				.expectStatus().isNotFound();
	}
	...
	void invalidOpenAssignBug() {
		invalidPostRequest(BUGS_OPEN_ASSIGN, new BugAssignDto(Seriousness.BLOCKING, DESCRIPTION, DATE_OPEN, -20));
	}
