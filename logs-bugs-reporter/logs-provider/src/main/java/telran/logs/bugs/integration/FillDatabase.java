...
public class FillDatabase {
	...
	String names[];
	@Value("${app-mail-account:reporter.logs.bugs}")
	String mailAccount;
	...
	private void fillArtifacts() {
		...
		for (int i = 1; i <= nArtifactsNofixed; i++) {
			artifactsForSaving.add(new Artifact(artifactBase + i, getProgrammer()));
		}
	...
	private Programmer getProgrammer() {
		Programmer programmer = programmerRepository.findById(getRandomLong(1, names.length + 1)).orElse(null);
		return programmer;
	}
	...
	private void fillProgrammers() {
		programmerId = 1;
		List<Programmer> programmersForSaving = Arrays.stream(names)
				.map(name -> new Programmer(programmerId, name, getEmail(programmerId++))).collect(Collectors.toList());
